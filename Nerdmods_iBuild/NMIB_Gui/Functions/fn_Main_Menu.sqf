...
_Content = switch (true) do
{
	case (_String == "STR_NMIB_Menu2_Txt"):
	{
		...
		_ActionKeyBind = if (actionKeys NMIB_ActionKey1 isEqualTo []) then {"Unbound"} else {(actionKeysNamesArray NMIB_ActionKey1 select 0)};
		...
		_PlotToggleKeyBind = if (actionKeys NMIB_ActionKey2 isEqualTo []) then {"Unbound"} else {(actionKeysNamesArray NMIB_ActionKey2 select 0)};
